...
const formSchema = z.object({
  ...
  previousLeaveId: z.string().optional(),
});
...
export function PermissionForm({ studentId, extendLeaveId, isLateSubmission, lateSubmissionType, onSuccess }: PermissionFormProps) {
  ...
  const [previousLeaves, setPreviousLeaves] = React.useState<LeaveRequest[]>([]);
  const [selectedPreviousLeave, setSelectedPreviousLeave] = React.useState<LeaveRequest | null>(null);
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      reasonType: "Sakit",
      duration: "1",
      startDate: isLateSubmission ? subDays(new Date(), 1) : new Date(), 
      reasonText: "",
  ...
  React.useEffect(() => {
    async function fetchInitialData() {
        ...
        if (lateSubmissionType === 'extend-late') {
  ...
  }, [studentId, extendLeaveId, isLateSubmission, lateSubmissionType]);
  ...
  React.useEffect(() => {
    if(watchPreviousLeaveId && lateSubmissionType === 'extend-late') {
  ...
  }, [watchPreviousLeaveId, lateSubmissionType, previousLeaves, form]);
  
  const handleDateSelect = (date: Date | undefined) => {
    if(!date || isExtensionMode || (lateSubmissionType === 'extend-late')) return;
    ...
    if (isLateSubmission) {
        ...
        if(date < today) {
            toast({
                variant: "destructive",
                title: "Tanggal Tidak Valid",
                description: "Tanggal yang Anda pilih berada di masa lalu. Silakan gunakan tombol 'Ajukan Izin Susulan' di dasbor Anda."
            })
            return;
        }
    }
  ...
  const whatsappMessage = React.useMemo(() => {
    ...
    const messageAction = isExtensionMode || lateSubmissionType === 'extend-late' 
      ? `memperpanjang izin${isLateSubmission ? ' (susulan)' : ''}` 
      : `tidak dapat masuk sekolah`;

    let message: string;

    if(durationDays === 1){
         message = `Assalamu'alaikum Wr. Wb.\n\nYth. Bapak/Ibu Wali Kelas ${values.studentClass}\n\nDengan ini kami beritahukan bahwa ananda ${values.studentName} ${messageAction} pada hari ${startDateFormatted} dikarenakan ${values.reasonType.toLowerCase()}.`;
    } else {
    ...
    return message;
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [form.watch(), isExtensionMode, isLateSubmission, lateSubmissionType]);
  ...
  const isLateExtension = isLateSubmission && lateSubmissionType === 'extend-late';
            ...
            <FormField
                ...
                <FormItem className="space-y-3">
                    ...
                    <RadioGroup
                        ...
                        disabled={isExtensionMode || isLateExtension}
                    >
                        <FormItem className="flex items-center space-x-3 space-y-0">
                        <FormControl>
                            <RadioGroupItem value="Sakit" disabled={isExtensionMode || isLateExtension} />
                        </FormControl>
                        <FormLabel className={cn("font-normal", (isExtensionMode || isLateExtension) && "text-muted-foreground")}>Sakit</FormLabel>
                        </FormItem>
                        <FormItem className="flex items-center space-x-3 space-y-0">
                        <FormControl>
                            <RadioGroupItem value="Izin" disabled={isExtensionMode || isLateExtension} />
                        </FormControl>
                        <FormLabel className={cn("font-normal", (isExtensionMode || isLateExtension) && "text-muted-foreground")}>Izin</FormLabel>
                        </FormItem>
                    </RadioGroup>
                    </FormControl>
                    <FormMessage />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                render={({ field }) => (
                    ...
                    <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
                        <PopoverTrigger asChild disabled={isExtensionMode || isLateExtension}>
                        <FormControl>
                            <Button
                            variant={"outline"}
                            className={cn(
                                ...
                                (isExtensionMode || isLateExtension) && "pointer-events-none bg-muted/50"
                            )}
                            >
                        ...
                        <Calendar
                            ...
                            disabled={(date) => isLateSubmission ? date >= new Date(new Date().setHours(0,0,0,0)) : date < new Date(new Date().setDate(new Date().getDate() - 1))}
                            initialFocus
                        />
                        </PopoverContent>
                    </Popover>
